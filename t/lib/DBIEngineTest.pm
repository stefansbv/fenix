...
sub run {
    ...
    subtest 'live database' => sub {
        ...
        isa_ok $target, 'App::Fenix::Target', 'target';
        ...
        throws_ok { $engine->dbh->do('INSERT blah INTO __bar_____') }
            'Exception::Db::SQL',
            'Database error should be converted to App::Fenix exception';
        ...
        ######################################################################
        if ($class eq 'App::Fenix::Engine::pg') {
...
END_SQL
        ...
        if ($class eq 'App::Fenix::Engine::sqlite') {
