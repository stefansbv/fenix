package App::Fenix::Config::Screen;
...
has 'scr' => (
    ...
    lazy     => 1,
    init_arg => undef,
    builder  => '_build_scrcfg',
);

sub _build_scrcfg {
    my $self = shift;
    my $scrcfg_file = $self->scrcfg_file->stringify;
...
sub screen {
    my ($self, @args) = @_;
    return Dive( $self->scr, 'screen', @args );
}

sub defaultreport {
    my ($self, @args) = @_;
    my $c = Dive( $self->scr, 'defaultreport', @args );
    return $c // {};
}

sub defaultdocument {
    my ($self, @args) = @_;
    my $c = Dive( $self->scr, 'defaultdocument', @args );
...
sub lists_ds {
    my ($self, @args) = @_;
    my $c =  Dive( $self->scr, 'lists_ds', @args );
    return $c // {};
}

sub lists_ds_tm {
    my ($self, @args) = @_;
    my $c = Dive( $self->scr, 'lists_ds_tm', @args );
    return $c // {};
}

sub list_header {
    my ($self, @args) = @_;
    return Dive( $self->scr, 'list_header', @args );
}

sub bindings {
    my ($self, @args) = @_;
    my $c = Dive( $self->scr, 'bindings', @args );
    return $c // {};
}

sub bindings_select {
    my ($self, @args) = @_;
    return Dive( $self->scr, 'bindings_select', @args );
}

sub tablebindings {
    my ($self, @args) = @_;
    my $c = Dive( $self->scr, 'tablebindings', @args );
    return $c // {};
}

sub maintable {
    my ($self, @args) = @_;
    return Dive( $self->scr, 'maintable', @args );
}

sub deptable {
    my ($self, @args) = @_;
    my $c = Dive( $self->scr, 'deptable', @args );
    return $c // {};
}
...
sub repotable {
    my ($self, @args) = @_;
    return Dive( $self->scr, 'repotable', @args );
}

sub scrtoolbar {
    my ($self, @args) = @_;
    my $c = Dive( $self->scr, 'scrtoolbar', @args );
    return $c // {};
}

sub toolbar {
    my ($self, @args) = @_;
    my $c =  Dive( $self->scr, 'toolbar', @args );
    return $c // {};
}
...
Load the screen configuration.
    ...
    use App::FenixConfig::Screen;

    my $foo = App::FenixConfig::Screen->new();
    ...
...
The application configuration instance object.
...
See the POD in L<setup_lookup_bindings_entry> in the App::FenixController
module.
...
See the POD in L<get_lookup_setings> in the App::FenixController
module.
