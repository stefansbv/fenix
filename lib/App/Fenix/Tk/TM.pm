...
# Copied here until I found a way to apply the role
sub sort_hash_by_id {
    my ( $self, $attribs ) = @_;
...
sub init {
    ...
    $self->{fields} = $self->sort_hash_by_id( $self->{columns} );
...
sub build_dateentry {
    ...
    my $button = $self->{frame}->DateEntry(
        ...
        -parsecmd        => sub {
            App::Fenix::Utils->dateentry_parse_date( 'iso', @_ );
        },
        -formatcmd => sub {
            my $date_str = App::Fenix::Utils->dateentry_format_date( $date_format, @_ );
