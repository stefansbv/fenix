...
sub query_records_count {
    ...
    my $sql = SQL::Abstract->new( special_ops => Fenix::Utils->special_ops );
...
sub build_sql_where {
    ...
    foreach my $field ( keys %{ $opts->{where} } ) {
        ...
        if ( $find_type eq 'contains' ) {
            ...
            my $ret = Fenix::Utils->process_date_string($searchstr);
            if ( $ret eq 'dataerr' ) {
...
sub tbl_dict_query {
    ...
    try {
        ...
        while ( my $row_rf = $sth->fetchrow_arrayref() ) {
            push @dictrows, {
                $label_lbl => Fenix::Utils->decode_unless_utf( $row_rf->[1] ),
                $value_lbl => Fenix::Utils->decode_unless_utf( $row_rf->[0] ),
            };
        }
    }
    catch {
...
sub table_batch_update {
    ...
    my $muc = Fenix::Model::Update::Compare->new(
        fk_col  => $fkcol,
        db_data => $oldrec,
        tm_data => $depdata,
    );
...
The first data structure it's a AoH to be displayed in the Fenix::TMSHR
widget.
